...
`ifndef RVGPU_GPC_L15_DATA_ARRAY_SV
`define RVGPU_GPC_L15_DATA_ARRAY_SV
...
`include "types_l15cache.svh"
`include "types_l15cache_data.svh"

module rvgpu_gpc_l15cache_data_array (
    ...
    // 数据数组相关参数
    localparam int L15CACHE_DATA_WIDTH = L15CACHE_LINE_WIDTH;
    localparam int L15CACHE_DATA_ADDR_WIDTH = L15CACHE_INDEX_BITS;
    localparam int L15CACHE_DATA_DEPTH = L15CACHE_SETS;
    localparam int L15CACHE_DATA_STATE_BITS = 2;
    ...
    // 状态机寄存器
    logic [L15CACHE_DATA_STATE_BITS-1:0] state_r, state_nxt;
    
    // 访问请求寄存器
    logic [L15CACHE_DATA_ADDR_WIDTH-1:0] access_index_r, access_index_nxt;
    logic [L15CACHE_WAYS-1:0] access_way_r, access_way_nxt;
    
    // 访问结果寄存器
    logic line_read_done_r, line_read_done_nxt;
    logic line_write_done_r, line_write_done_nxt;
    l15cache_line_t line_read_data_r, line_read_data_nxt;
    
    // SRAM接口信号
    logic [L15CACHE_WAYS-1:0] sram_ce_r, sram_ce_nxt;
    logic [L15CACHE_WAYS-1:0] sram_we_r, sram_we_nxt;
    logic [L15CACHE_DATA_ADDR_WIDTH-1:0] sram_addr_r, sram_addr_nxt;
    logic [L15CACHE_DATA_WIDTH-1:0] sram_wdata_r, sram_wdata_nxt;
    logic [L15CACHE_DATA_WIDTH-1:0] sram_rdata [L15CACHE_WAYS];
    
    // Way索引转换函数
    function automatic logic [2:0] way_to_index(input logic [L15CACHE_WAYS-1:0] way_vector);
        logic [2:0] result;
        result = 3'b000;
    ...
    // SRAM接口实例数组
    rvgpu_sram_if #(
        .WIDTH(L15CACHE_DATA_WIDTH),
        .HEIGHT(L15CACHE_DATA_DEPTH)
    ) sram_if_inst [L15CACHE_WAYS]();
    
    // 连接时钟
    for (genvar i = 0; i < L15CACHE_WAYS; i++) begin : gen_sram_clk
        assign sram_if_inst[i].clk = clk;
    end
    
    // SRAM实例数组
    for (genvar i = 0; i < L15CACHE_WAYS; i++) begin : gen_sram
        rvgpu_sram_sp #(
            .WIDTH(L15CACHE_DATA_WIDTH),
            .HEIGHT(L15CACHE_DATA_DEPTH),
            .RAMNAME("L15CACHE_DATA_SRAM")
        ) u_data_sram_way (
    ...
    always_comb begin : comb_logic
        ...
        // 接口输出默认值
        data_if.line_read_ready = (state_r == L15CACHE_DATA_STATE_IDLE);
        data_if.line_write_ready = (state_r == L15CACHE_DATA_STATE_IDLE);
        data_if.line_read_done = line_read_done_r;
        data_if.line_read_data = line_read_data_r;
        data_if.line_write_done = line_write_done_r;
        
        case (state_r)
            L15CACHE_DATA_STATE_IDLE: begin
                // 空闲状态：等待新请求
                ...
                if (data_if.line_read_valid && data_if.line_read_ready) begin
                    // 开始缓存行读操作
                    state_nxt = L15CACHE_DATA_STATE_LINE_READ;
                    ...
                    // 开始缓存行写操作
                    state_nxt = L15CACHE_DATA_STATE_LINE_WRITE;
            ...
            L15CACHE_DATA_STATE_LINE_READ: begin
                // 缓存行读操作状态 - 发起SRAM读取
                sram_ce_nxt = '0; // 停止SRAM访问
                state_nxt = L15CACHE_DATA_STATE_LINE_READ_WAIT;
            end
            
            L15CACHE_DATA_STATE_LINE_READ_WAIT: begin
                // 缓存行读等待状态 - 等待SRAM读取完成
                sram_ce_nxt = '0; // 停止SRAM访问
                ...
                // 返回空闲状态
                state_nxt = L15CACHE_DATA_STATE_IDLE;
            end
            
            L15CACHE_DATA_STATE_LINE_WRITE: begin
                // 缓存行写操作状态
                sram_ce_nxt = '0; // 停止SRAM访问
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            // 复位逻辑
            state_r <= L15CACHE_DATA_STATE_IDLE;
            access_index_r <= '0;
            access_way_r <= '0;
...
endmodule : rvgpu_gpc_l15cache_data_array

`endif // RVGPU_GPC_L15_DATA_ARRAY_SV 