...
interface gpc_mmu_noc_if;
    ...
    // 模块端口
    modport gpc_mmu (
        output req_valid, req_vaddr, req_type, req_warp_id, req_source_id, req_gpc_id, resp_ready,
        input  req_ready, resp_valid, resp_ppn, resp_fault, resp_warp_id, resp_source_id
    );
    
    modport noc_adapter (
        input  req_valid, req_vaddr, req_type, req_warp_id, req_source_id, req_gpc_id, resp_ready,
        output req_ready, resp_valid, resp_ppn, resp_fault, resp_warp_id, resp_source_id
    );
    
    // 任务和函数
    // GPC MMU使用的任务
    task mmu_send_request(
        input logic [38:0] vaddr,
        input logic [2:0]  type,
        input logic [31:0] warp_id,
        input logic [3:0]  source_id,
        input logic [7:0]  gpc_id
    );
        req_valid <= 1'b1;
        req_vaddr <= vaddr;
        req_type <= type;
    ...
    // NOC Adapter使用的任务
    task noc_accept_request();
        req_ready <= 1'b1;
        @(posedge req_valid);
        req_ready <= 1'b0;
    endtask
    
    task noc_send_response(
