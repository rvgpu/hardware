...
module rvgpu_job_dispatcher #(
    ...
    logic [`CONFIG_GPC_NUMBER-1:0] gpc_busy_r, gpc_busy_n;  // 修改：GPC 忙状态
    logic [$clog2(`CONFIG_GPC_NUMBER)-1:0] gpc_sel_r, gpc_sel_n;  // 修改：GPC 选择
    ...
    function automatic noc_node_id_t next_gpc_sel(input logic [`CONFIG_GPC_NUMBER-1:0] busy_vec, input int last_sel);
        int i;
        for (i = 1; i <= `CONFIG_GPC_NUMBER; i++) begin
            int idx = (last_sel + i) % `CONFIG_GPC_NUMBER;
